function main() {
  ...
  sheet.getDataRange().getValues().slice(ROW_HEADER).forEach(v => {
    ...
    const messages: string[] = connpass.events.map(event => { return `${event.title}\n${event.event_url}` });
    if (messageInfos.length === 0) {
      sheetRow.slackIds.forEach(slackId => {
        messageInfos.push(new MessageInfo(slackId, [sheetRow.keyword], messages));
      });
    } else {
      sheetRow.slackIds.forEach(slackId => {
        messageInfos.forEach((messageInfo, i) => {
          if (messageInfo.slackId === slackId) {
            ...
            messageInfos[i].message.push(...messages.filter(message => messageInfos[i].message.indexOf(message) === -1))
          } else {
            messageInfos.push(new MessageInfo(slackId, [sheetRow.keyword], messages));
          }
        });
  ...
  messageInfos.forEach(messageInfo => {
    ...
    callSlackWebhook(messageInfo.slackId, properties.SLACK_WEBHOOK_URL, message);
  });
}